...
export function TableList({ onInsert }: TableListProps) {
  ...
  return (
    <div className="border-t border-gray-200 dark:border-gray-700 pt-2 mt-2" data-testid="table-list">
      <h4 className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase mb-1 px-1">Tables</h4>
          ...
          return (
            <button
              ...
              className={`w-full text-left px-2 py-1 text-xs rounded hover:bg-gray-100 dark:hover:bg-gray-700 ${
                isActive ? 'text-blue-700 dark:text-blue-400 font-medium' : 'text-gray-600 dark:text-gray-300'
              }`}
              title={`Click to insert "${tableName}"`}
              data-testid={`table-${sheet.id}`}
            >
              <span className="font-mono">{tableName}</span>
              {isActive && <span className="ml-1 text-gray-400 dark:text-gray-500">(current_sheet)</span>}
            </button>
          );
        })}
      </div>
