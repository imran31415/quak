...
describe('AI tools — end-to-end parity with HTTP routes', () => {
  ...
  const TEST_USER_ID = 'test-user-aitools';

  // Wrapper so the existing assertions don't have to thread userId everywhere.
  async function tool(name: string, args: Record<string, unknown>) {
    return executeTool(name, args, TEST_USER_ID);
  }
  ...
  beforeAll(async () => {
    ...
    await initDb();
    // Seed a test user so create_sheet's owner_id FK is satisfiable.
    await getDb().run(
      `INSERT INTO __quak_users (id, display_name) VALUES ('${TEST_USER_ID}', 'Test User')`,
    );
  });
  ...
  it('create_sheet table includes __order so GET /api/sheets/:id ORDER BY __order works', async () => {
    const result = await tool('create_sheet', {
      name: 'Test Sheet',
      columns: [
  ...
  it('add_rows populates __order so rows are returned in insert order', async () => {
    const create = await tool('create_sheet', {
      name: 'Ordered',
  ...
  it('sort_sheet updates __order so the new order is visible via GET /api/sheets/:id', async () => {
    ...
    await tool('sort_sheet', { sheetId: sheet.id, column: 'N', direction: 'asc' });
  ...
  it('list_sheets returns sheets with parsed column metadata', async () => {
    await tool('create_sheet', {
      name: 'Listable',
      columns: [{ name: 'X', cellType: 'text' }],
    });
    const result = await tool('list_sheets', {});
  ...
  it('get_sheet returns metadata + rows for an existing sheet', async () => {
    ...
    await tool('add_rows', { sheetId, rows: [{ Item: 'X' }] });

    const result = await tool('get_sheet', { sheetId });
  ...
  it('get_sheet errors on nonexistent sheet id', async () => {
    const result = await tool('get_sheet', { sheetId: 'does-not-exist' });
  ...
  it('update_cells writes the new value identified by rowid', async () => {
    ...
    const sheetId = (create.data as { id: string }).id;
    await tool('add_rows', { sheetId, rows: [{ Item: 'A', Qty: 1 }, { Item: 'B', Qty: 2 }] });

    const result = await tool('update_cells', {
      sheetId,
  ...
  it('delete_rows removes the given rowids from the table', async () => {
    ...
    const sheetId = (create.data as { id: string }).id;
    await tool('add_rows', { sheetId, rows: [{ X: 'a' }, { X: 'b' }, { X: 'c' }] });

    // DuckDB rowids start at 0, so rowIds [1, 2] = 'b' and 'c', leaving 'a'.
    await tool('delete_rows', { sheetId, rowIds: [1, 2] });
  ...
  it('add_column adds the column to both the table and the metadata', async () => {
    ...
    await tool('add_column', { sheetId, name: 'B', cellType: 'number' });

    // Metadata
    const meta = await tool('get_sheet', { sheetId });
  ...
  it('rename_column renames in both physical table and metadata; data preserved', async () => {
    ...
    const sheetId = (create.data as { id: string }).id;
    await tool('add_rows', { sheetId, rows: [{ OldName: 'value' }] });

    await tool('rename_column', { sheetId, columnId: 'oldname', newName: 'NewName' });
  ...
  it('delete_column drops the column from table and metadata', async () => {
    ...
    await tool('delete_column', { sheetId, columnId: 'b' });
  ...
  it('delete_sheet drops the table and removes metadata', async () => {
    ...
    await tool('delete_sheet', { sheetId });
  ...
  it('run_sql allows SELECT but rejects mutating statements', async () => {
    const ok = await tool('run_sql', { sql: 'SELECT 1+1 AS two' });
    const okData = ok.data as { rows: Array<Record<string, unknown>> };
    expect(Number(okData.rows[0].two)).toBe(2);

    const bad = await tool('run_sql', { sql: 'DROP TABLE __quak_sheets' });
  ...
  it('summarize_data returns row count and per-column stats', async () => {
    ...
    const result = await tool('summarize_data', { sheetId });
  ...
  it('filter_sheet filters by equality, contains, and numeric comparison', async () => {
    ...
    const eq = await tool('filter_sheet', { sheetId, column: 'Name', operator: 'equals', value: 'Alice' });
    expect((eq.data as { rowCount: number }).rowCount).toBe(1);

    const contains = await tool('filter_sheet', { sheetId, column: 'Name', operator: 'contains', value: 'a' });
    ...
    const gt = await tool('filter_sheet', { sheetId, column: 'Score', operator: 'greater_than', value: '60' });
  ...
  it('set_conditional_format attaches rules to the column metadata', async () => {
    ...
    await tool('set_conditional_format', {
      sheetId,
      columnId: 'score',
